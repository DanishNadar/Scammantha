...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    let { response, data } = await requestChat(model || CHAT_MODEL_DEFAULT, messages);
    if (!response.ok) {
...
app.listen(PORT, () => console.log(`Scammantha local server running on http://localhost:${PORT}`));
